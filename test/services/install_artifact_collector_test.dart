...
void main() {
  group('InstallArtifactCollector', () {
    test('tüm teşhis komutları çalıştırılır ve loglanır', () async {
      ...
      final fullCmds = fake.commandLog
      ...
      expect(
        fullCmds.any((c) => c.startsWith('cat /mnt/etc/kernel/cmdline')),
        true,
      );

      // ls komutları
      expect(
        fullCmds.any((c) => c.startsWith('ls -la /mnt/boot/efi/EFI/fedora/')),
        true,
      );
      expect(
        fullCmds.any((c) => c.startsWith('ls -la /mnt/boot/loader/entries/')),
        true,
      );

      // Log kontrolü
      expect(
        loggedMessages.any((m) => m.contains('KURULUM HATASI TEŞHİS RAPORU')),
        true,
      );
